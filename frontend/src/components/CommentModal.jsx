...
const CommentModal = ({ song, user, onClose }) => {
  ...
  useEffect(() => {
    if (song) {
      const fetchComments = async () => {
        setIsLoading(true);
        try {
          const response = await api.get(`/comments/${song._id}`);
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const response = await api.post(`/comments/${song._id}`, { text: newComment });
      setComments([response.data, ...comments]);
      setNewComment('');
    } catch (error) {
      console.error("Failed to post comment", error);
      alert("Could not post comment. Please make sure you are logged in.");
    }
  };
                ...
                <div key={comment._id} className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-purple-600 rounded-full flex items-center justify-center font-bold shrink-0">
                    {comment.user && comment.user.profileImage ? (
                      <img src={comment.user.profileImage} alt={comment.user.username} className="w-full h-full object-cover rounded-full" />
                    ) : (
